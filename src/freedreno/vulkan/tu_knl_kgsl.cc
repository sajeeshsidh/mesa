...
static VkResult
bo_init_new_ion_dmaheap(struct tu_device *dev, struct tu_bo **out_bo, uint64_t size,
                enum tu_bo_alloc_flags flags)
{
   ...
   int ret;
   ret = safe_ioctl(dev->physical_device->kgls_dma_fd, DMA_HEAP_IOCTL_ALLOC,
                    &alloc);
                ...
                enum tu_bo_alloc_flags flags)
{
   ...
   int ret;
   ret = safe_ioctl(dev->physical_device->kgls_dma_fd, ION_IOC_NEW_ALLOC, &alloc);
   if (ret) {
      return vk_errorf(dev, VK_ERROR_OUT_OF_DEVICE_MEMORY,
                       ...
                       enum tu_bo_alloc_flags flags)
{
   ...
   int ret;
   ret = safe_ioctl(dev->physical_device->kgls_dma_fd, ION_IOC_ALLOC, &alloc);
   if (ret) {
      return vk_errorf(dev, VK_ERROR_OUT_OF_DEVICE_MEMORY,
   ...
   ret = safe_ioctl(dev->physical_device->kgls_dma_fd, ION_IOC_SHARE, &share);
   if (ret) {
      return vk_errorf(dev, VK_ERROR_OUT_OF_DEVICE_MEMORY,
   ...
   ret = safe_ioctl(dev->physical_device->kgls_dma_fd, ION_IOC_FREE, &free);
   if (ret) {
      return vk_errorf(dev, VK_ERROR_OUT_OF_DEVICE_MEMORY,
             ...
             const char *name)
{
   ...
   if (flags & TU_BO_ALLOC_SHAREABLE) {
      ...
      case TU_KGSL_DMA_TYPE_DMAHEAP:
         return bo_init_new_ion_dmaheap(dev, out_bo, size, flags);
