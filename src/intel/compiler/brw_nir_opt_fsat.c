 ...
 *    the worklist. If a uses is a fsat instruction or a phi node, add the
...
fixup_defs(struct set *fixup)
{
   ...
   set_foreach_remove(fixup, entry) {
      ...
      /* WARNING: nir_def_rewrite_uses CANNOT be used here because new_fsat is
       * a user of src.
       */
      nir_foreach_use_safe(use_src, src_def) {
         if (nir_src_parent_instr(use_src) != new_fsat->parent_instr) {
            nir_src_rewrite(use_src, new_fsat);
         }
      }
   }
}
